...
DMA_Res <- DMA(
    InputData = InputData,
    SettingsFile_Sample = as.data.frame(colData(metabolomics)),
    SettingsInfo = c(Conditions ="tissue", Numerator = "TU" , Denominator = "NAT"),
    SettingsFile_Metab = as.data.frame(rowData(metabolomics)),
    StatPval = "t.test",
    StatPadj = "fdr",
    PerformShapiro = TRUE,
...
## load Biocrates IDs:
FeatureMetadata_Biocrates <- ToyData(Data="BiocratesFeatureTable")
...
Plotdf <- data.frame(
    ...
    mutate(
        ...
        Class = as.factor(SettingsFile_Metab$Class)  
    )

Fig4A <- MetaProViz:::GenerateUpset(
    df = Plotdf,
    class_col = "Class",
...
Gaude_Pathways <- LoadGaude()
Gaude_GeneMetab <- Make_GeneMetabSet(Input_GeneSet = Gaude_Pathways,
    SettingsInfo = c(Target = "gene"),
    PKName = "Gaude")
MetaLinksDB_Res <- LoadMetalinks()
```
