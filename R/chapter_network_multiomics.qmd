...
4. If there are missing values (NAs) in the data linear modeling can be used, such as limma, which fits a linear model to the data and can handle missing values [@Ritchie_2015]. Yet, the limma method assumes that the residuals of the linear model are approximately normally distributed [@Ritchie_2015]. Other options are to either perform missing value imputation, which comes with it's own pitfalls in metabolomics, or to assume that the missing values are true zeros, which in turn can lead to zero inflation [@Huang_2022].

Here we use the R package [MetaProViz](https://saezlab.github.io/MetaProViz/index.html) to perform differential analysis since it automatically performs both, the Shapiro test for normality and the Bartlett's test for homoscedasticity, and gives the choice between different statistical test [@Schmidt2025]. Importantly, MetaProViz also adjusts for multiple testing using Benjamini Hochberg, False Discovery Rate (FDR), Bonferroni or Holm [@Schmidt2025].
...
The Shapiro test results shows that the majority of the metabolites follow mostly a normal distribution, which means it is appropriate to perform a t-test (Fig.3a-b). Yet, it is important to note that almost half of the metabolites are not normally distributed, which means that the t-test may not be appropriate for these metabolites. In this case, it might be better to use a non-parametric test such as the Wilcoxon rank-sum test or Kruskal-Wallis test. 
```{r}
#| eval: true
#| echo: false
#| message: false
...
Displaying all metabolite classes on the volcano plot can make it difficult to interpret (Fig. 3d) and it can be useful to create individual plots for each of the metabolite class for better interpretability (Fig. 3f-h).
```{r}
#| eval: true
#| echo: false
#| message: false
