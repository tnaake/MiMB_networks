...
## COMMENT: still not the "old" abundance ranges, volcano plot seems off
...
#1. metabolite-sets
KEGG <- MetaProViz::LoadKEGG()

ChemicalClass <- MetaProViz::LoadRAMP()

#2. gene-metabolite sets
Gaude_Pathways <- MetaProViz::LoadGaude()
...
#| results: false
#| fig.show: hide
DMA_Res_wilcox[["DMA"]][["TU _vs_ NAT"]] |> ## COMMENT: why not assigned to InputData?
    tibble::column_to_rownames("Metabolite") |>
...
DetectedIDs <- InputData |>
  ...
  tidyr::drop_na()|>
      ...
      tidyr::separate_rows("HMDB", sep = ",\\s*") |>
      group_by(row_id) |>
      mutate("OriginalEntry_InputData" := paste0("InputData_", dplyr::cur_group_id())) |>
      ungroup()
...
FactorsProt <- merge(x = FactorsProt,
        ...
        by = 0,
        all.x = TRUE) |>
    dplyr::mutate(gene = stringr::str_extract(as.character(fasta_header), "(?<=GN=)[^ ]+"))|>
    dplyr::filter(!is.na(gene)) |>
    dplyr::group_by(gene) |>
    dplyr::slice_max(order_by = abs(Factor1), n = 1, with_ties = FALSE) |>
    dplyr::ungroup()
...
decoupler_res <- decoupleR::run_ulm(
    mat = as.matrix(Input), 
    net=Gaude_Pathways, 
    .source = "term", 
    .target = "gene",
...
D: Thresholded network based on Pearson and Spearman correlation. Nodes represent metabolites and are colored according by community membership based on the Infomap community detection algorithm. 
