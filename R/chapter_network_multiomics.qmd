...
1. Collections of biological pathways or processes meaning sets of genes/metabolites with common characteristic (e.g. GO-term, KEGG,etc.), which are usable for classical mapping. Here the dysregulation of a pathway is inferred from measurements of its own components (e.g. metabolite abundance).
2. Collections of enzymes/molecules and their targets meaning for example kinases and target phosphopeptides, which are usable for footprinitng analysis. Footpriting analysis estimates the activities from molecular readouts of the targets (e.g. phosphosites) downstream of the enzyme (e.g. kinase). (REF: https://www.sciencedirect.com/science/article/pii/S2452310019300149, )
...
It becomes apparent that for some metabolite classes only certain metabolite ID types are available (Figure 4a). This means, if for example Reactome [@REF], which uses chEBI IDs is used for pathway enrichment analysis, only a small subset of the detected metabolites will be included, which needs to be taken into consideration when interpreting the results biologically. 
...
Even for gene-sets, there are sets with a strong focus on metabolism available. Indeed, the gene-sets generated by Gaude et. al. [@REF]
- Gaude for proteomics --> focus on metabolic enzymes!


***3. Biological interpretation of the results***

-Perform ORA and plot VolcanoPlot
- Parameters like pathway size and significance thresholds need to be taken into account.
...
- Showcase one example? MetalinksDB to combine with proteomics. 
...
#row2 <- plot_grid(Fig3C, Fig3D, NULL, ncol = 3, labels = c("C", "D"))
#row3 <- plot_grid(Fig3E, Fig3F, Fig3G, ncol = 3, labels = c("E", "F", "G"))
g <- plot_grid(row1, ncol = 1) #rel_heights = c(0.7, 1, 1))

ggsave(filename = "figure/figure4.jpg", plot = g, device = "jpg", dpi = 600)
```
