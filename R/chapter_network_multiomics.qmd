...
2. Choose a statistical method to be applied on the PK taking into account
   ...
   [@BadiaIMompel2022]. In a benchmark study [@BadiaIMompel2022] linear models
   (e.g. ULM, MLM, etc.) outperformed GSEA, whilst ORA performs similarly well
   evaluated on transcriptomic and phospho-proteomic perturbation
   experiments[@BadiaIMompel2022]. 

### Annotated metabolites and coverage in the prior knowledge

Pathway analysis was originally developed for gene expression data. Even though 
it was adapted to metabolomics data, it often produces misleading or 
nonsensical results due to the unique characteristics and constraints of metabolites compared to genes [@Lee2025]. 
...
1. Choose a database for metabolite IDs. Different to gene names, there are no
 ...
 <!-- COMMENT: Add here code-->

2. Curate metabolite ID mappings. 
<!-- COMMENT: Is curate a good name? or harmonize? -->
   The database often include multiple entries for the same metabolite 
   with different degrees of ambiguity [@Pham2019], as for example KEGG, which
   includes entries for L-Alanine, D-Alanine and Alanine. 
<!-- COMMENT: is the example for KEGG needed? -->
   Experimentally different degrees of ambiguity are needed due to the machine
   sensitivity, where for example stereoisomers are not distinguished 
   [@Dias2016]. Yet, standard metabolite-sets used as prior knowledge often
...
3. Visualize the available metabolite IDs for the detected 
   ...
   of choice we recommend assignment of metabolite IDs to the detected
   metabolites.
   
   <!-- COMMENT: What does the the last sentence mean? It sounds unclear. Could you clarify? -->
...
- For classical mappings, obtain metabolite sets from databases such as KEGG,
  Reactome or WikiPathways, *RaMP-DB*, *Omnipath*, *MetSigDB* 
  (*choice of user*). 
  *RaMP-DB* [@Braisted2023] has combined many classical resources trying to 
  account for mismappings between the original resources by using molecular
  weight and has used *ClassyFire* [@DjoumbouFeunang2016] to assign chemical classes to the metabolites. 
  *Omnipath* [@Türei2016, @Türei2021] a database of molecular biology prior
  ...
  identifier translation. These classical databases
  mainly carry information about the metabolic pathway and the metabolites 
  that are assigned to them. 

- Chemical class-metabolite sets can be used for classical mapping to 
  understand if certain compound classes are affected in the comparison of
  interest [@Braisted2023]. 
    <!-- COMNENT: this needs more description, so this contains the map metabolite-class= -->
- To explore signalling function of metabolites by binding 
  to receptors, choose a database such as *MetalinksDB* [@Farr&Dimitrov2024].
  *MetalinksDB* contains 
  information about metabolite-receptor and metabolite-transporter interaction,
  as well as weather the binding of the metabolite to a receptor is activating 
  ...
  highlighting the signalling potential of the metabolites [@Schmidt2025].
  *MetaProViz* includes a collection of annotated
  ...
  *MetSigDB*), which includes classical pathway-metabolite sets, chemical
  ...
  sets [@Schmidt2025]. Moreover, *MetaProViz* uses the *COSMOS* enzyme-metabolic
...
- When working with human data, the prior knowledge network 
...
- If no metabolomics data are available, but the focus is still on metabolic
...
2. (recommended) Exclude certain metabolites from database. The same 
   ...
   <!-- COMMENT: What is missing here are code examples. I would suggest to pick one database that is suitable for the data here and prepare the data according to 1 and 2. -->
...
<!-- COMMENT: Should point 6. be moved to above? -->
