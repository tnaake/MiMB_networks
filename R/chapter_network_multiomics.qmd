...
The Shapiro test results shows that the majority of the metabolites follow a normal distribution, which means it is appropriate to perform a t-test (Fig.3a-b). 
```{r}
#| eval: true
#| echo: false
#| message: false
...
Moreover, checking the results of the differential analysis by plotting a Volcano plot, we can see the proportion of the metabolites that are significantly up- or down-regulated between the two conditions (Fig.3c). 
```{r}
#| eval: true
#| echo: false
#| message: false
...
Fig3C <- DMA_Res[["VolcanoPlot"]][["TU__vs__NAT"]]
...
Dsiplaying all metabolite classes on the volcano plot can make it difficult to interpret (Fig. 3d). For better visibility individual plots for each of the metabolite class can be created (Fig. 3E-G).
```{r}
#| eval: false
#| echo: false
#| message: false
...
Fig3E <- Res[["Plot_Sized"]][["Ceramides"]]
Fig3F <- Res[["Plot_Sized"]][["Sphingolipids"]]
Fig3G <- Res[["Plot_Sized"]][["Triacylglycerols"]]  
```
...
row2 <- plot_grid(Fig3C, Fig3D, NULL, ncol = 3, labels = c("C", "D"))
...
1. Collections of biological pathways or processes meaning sets of genes/metabolites with common characteristic (e.g. GO-term, KEGG,…), which are usable for classical mapping, where the dysregulation of a pathway is inferred from measurements of its own components (e.g. protein expression).
2. Collections of enzymes/molecules and their targets meaning for example kinases and target phosphopeptides, which are usable for footprinitng analysis, which estimates the activities from molecular readouts of the targets (e.g. phosphosites) downstream of the enzyme (e.g. kinase). (REF: https://www.sciencedirect.com/science/article/pii/S2452310019300149, )
...
Looking into the example metabolites from Table 1 it becomes clear that for some metabolites multiple ChEBI and HMDB IDs are available, whilst for other metabolite those databases have no entry. To systematically look into this, we can visualize the available IDs for the detected metabolites in our example data:
```{r}
#| eval: true
#| echo: false
#| message: false
...
UpsetPlot <- MetaProViz:::GenerateUpset(df = Plotdf,
                                        class_col = "Class",
...
From the upset plot it becomes clear that for some metabolite classes only certain metabolite ID types are available. This means, if for example Reactome [@REF], which uses chEBI IDs is used for pathway enrichment analysis, only a small subset of the detected metabolites will be included, which needs to be taken into consideration when interpreting the results biologically. 
...
To use these different databases the original resource can be downloaded, but there are also more convenient solutions. Indeed, RaMP-DB has combined many classical resources trying to account for miss-mapping between the original resources by using molecular weight [@REF]. 

- Cite MetaProViz, Liana for access to MetaLinkDB
- Mention the conversion of gene-sets to gene-metabolite sets using cosmos (Cite cosmos)
- Also mention Ominpath-metabo to access many of those resources. Whilst MetSigDB provides versions of the prior knowledge in a format usable for classical mapping, Ominpath-metabo provides the full breath of available information and enables user-specific selections. 

- ID problem - inflate/deflate (When needing to translate IDs of pathways), also in regard to gene-metab


***3. Biological interpretation of the results***

-Perform ORA and plot VolcanoPlot


- https://pubmed.ncbi.nlm.nih.gov/34492007/
- https://www.nature.com/articles/s42255-025-01283-0 (new paper also cite in MetaProViz!)

- showcase coverage of Biocrates (upsetplot?)

- metabolomics discussion inflation deflation and coverage in prior knowledge.

- MetalinksDB to combine with proteomics. 

discuss progeny transcriptomics (perturbations mit signatures) --> progeny is weighted!
Gene set enrichment kann auch footprint method sein (e.g. Progeny)

mention where PK can be accessed: Omnipath, MetSigDB, MSigDB



Christina: Structure to work on:
1. Explain the type of prior knowledge avaialable and dependent on this, which analysis can be doen (i.e. pathway enrichment, footprinting, network analysis)
2. difference metabolomics other omics 
3. metabolite IDs
4. Discuss: Parameters like pathway size and significance thresholds should be adjusted based on the dataset.

include: PROGENY, Hallmarks, Gaude for proteomics
include: KEGG, chemcial class for metabolomics
include Metalinks-db and gene-metab set (Gaude) for combine enrichment analysis

In the context of metabolomics and proteomics, prior knowledge can take various forms, including pathway databases, gene sets, and chemical classifications. These resources can be used for pathway enrichment analysis, network analysis, and other types of functional annotation.

mention cornetto for metabolic networks
<!-- !!! Christina, to continue !!! -->


<!-- !!! Thomas, I left this here, please check if you want to move this elsewhere. -->
