...
2. Choose a statistical method to be applied on the PK taking into account
   ...
   is a combination of change and statistics, will alter the GSEA results.

Here it will be crucial to understand if performing classical enrichment analysis 
will be beneficial for the biological question at hand. Metabolites are not genes, 
...
3. Visualize the available metabolite IDs for the detected 
   ...
   to ensure the maximum overlap between measured metabolites and PK
   of choice we recommend assignment of multiple metabolite IDs dependent 
   on the level of ambiguity to the detected
   metabolites.
...
- For classical mappings with metabolite sets, obtain PK from 
  databases such as *KEGG*, *Reactome* or *WikiPathways*, *RaMP-DB*, *Omnipath*, or
  *MetSigDB* (*choice of user*): 
  ...
  *MetSigDB*), which includes classical pathway-metabolite sets, chemical
  ...
  reaction PKN [@Dugourd_2021] to convert any gene set to
...
2. (recommended) Exclude certain metabolites from database. The same 
   ...
   enrichment analysis. *MetaProViz* provides already pathway-metabolite sets 
...
SettingsFile_Metab <- merge(
        x = as.data.frame(rowData(metabolomics))[, c(1:2)],
        y = FeatureMetadata_Biocrates[, c(2:3, 5, 7, 11)],
        by.x = "feature_original",
        by.y = "TrivialName_Prior2023",
        all.x = TRUE) |>
...
plot(g_keep, vertex.label = NA, vertex.size = 3, legend = TRUE, layout = layout_g_keep)
