...
row2 <- plot_grid(Fig3C, Fig3D, Fig3E, ncol = 3, labels = c("C", "D", "E"),  rel_widths = c(0.85, 0.85, 1.3) )
row3 <- plot_grid( Fig3F, Fig3G, Fig3H,NULL, ncol = 4, labels = c("F", "G", "H"),  rel_widths = c(0.85, 0.85, 0.85, 0.45) )
...
prior knowledge, which can be divided in several steps:
...
2. Investigate metabolite ID mappings. 
   ...
   metabolite-sets used as prior knowledge often
...
Table 1: **Preview of selected metabolites to showcase ChEBI and HMDB information provided by Biocrates. Biocrates results provide all possible identifiers for each trivial metabolite name. For some metabolites multiple ChEBI and HMDB IDs are available, whilst for other metabolite those databases have no entry.** 
...
3. Visualize the available metabolite IDs for the detected 
   ...
   types for metabolite classes (Figure 4 A). For instance, for Reactome [@Gillespie_2022], which uses ChEBI IDs for pathway enrichment analysis, only a
   ...
   of choice we recommend assignment of multiple metabolite IDs dependent on the level of ambiguity to the detected
   metabolites.
...
4. Given the incomplete coverage in mass spectrometry-based metabolomics and hence the potential low coverage of the prior knowledge, adjust the choice of statistical tests. For metabolomics, methods such as ORA should generally perform well since the method offers the ability to restrict the background to the detected metabolites (assay-specific background) [@Wieder2021].

### Available prior knowledge for metabolites

1. Choose a suitable database containing prior knowledge:

- For classical mappings with metabolite sets, those can be obtained from 
  databases such as KEGG, Reactome or WikiPathways, *RaMP-DB*, *Omnipath*, *MetSigDB* 
  (*choice of user*): 
  ...
  weight. Apart from classical pathway-metabolite sets such as wikipathways, *RaMP-DB* [@Braisted2023] 
  ...
  b) *Omnipath* [@Türei2016, @Türei2021] a database of molecular biology prior
  knowledge combines data from more than 1000 resources and incorporates many
  database clients containing metabolites and utilities for metabolite 
  identifier translation. Hence, apart from pathway-metabolite sets *Omnipath metabo*
  also offers many other ontology information.
  c) To explore signalling function of metabolites by binding 
  to receptors, you can use *MetalinksDB* [@Farr&Dimitrov2024].
  *MetalinksDB* contains information about metabolite-receptor and metabolite-transporter 
  interaction, as well as weather the binding of the metabolite to a receptor is activating 
  ...
  *MetSigDB*), which includes classical pathway-metabolite sets, chemical
  ...
  e) When working with human data, the prior knowledge network 
...
2. (recommended) Exclude certain metabolites from database. The same 
   ...
   enrichment analysis. *MetaProViz* has already done this and provides pathway-
   metabolite sets without unspecific metabolites [@Schmidt2025].
...
2. Understand the coverage of your experimental data in the prior knowledge resource. If the prior knowledge only includes metabolites that are available in, for example, HMDB, this would in turn exclude many detected ceramides, glycosylceramides and triglycerols, whilst using ChEBI IDs would exclude almost all triacylglycerols (Figure 4 A-D). Here, triacylglycerols include 226 metabolites of which only 11 have a ChEBI ID (Figure 4 D). If the results of the enrichment analysis do not show those pathways as significantly altered, it does not mean that they are not altered, but rather that the prior knowledge used is not able to capture them. Consequently, it is important not to over-interpret the importance of the most altered pathways, but rather to use them as a starting point for further investigation not ignoring the detected metabolites that were not covered by the prior knowledge resource. 

3. (optional) Combine multiple prior knowledge resources to increase the coverage of the detected metabolites.

4. Check how many metabolites of e.g. a pathway in the prior knowledge have been detected. For example, if a pathway only contains 20 metabolites and 13 of them are detected (65%), this is a stronger evidence than detecting 40 metabolites of a pathway containing 110 metabolites (36%). 
...
#| fig.show: "hide"
  ...
  DMA_Res_wilcox[["DMA"]][["TU _vs_ NAT"]] |>
  tibble::column_to_rownames("Metabolite") |>
...
#If you seek to use for example the MOFA results instead of the differential analysis, you can do this by:
#Extracting Factor of interest (here Factor 1 as it separates Tumour and Normal well)
...
#Merging with the feature metainformation needed to integrate with prior knowledge:
...
#Now we can for example use decoupleR and the Gaude pathways to perform enrichment analysis of Factor1 proteins for metabolic enzymes:
...
decoupler_res <- decoupleR::run_ulm(mat=as.matrix(Input), 
                                          ...
                                          .target='gene',
                                          .mor='weight', 
                                          minsize = 5) # Minimum of target features per biological entity. 
...
row1 <- plot_grid(Fig4A, Fig4B, ncol = 2, labels = c("A", "B"), rel_widths = c(1.6, 1.2))
row2 <- plot_grid(Fig4C, Fig4D, ncol = 2, labels = c( "C", "D"), rel_widths = c(1.6, 1.2) )
